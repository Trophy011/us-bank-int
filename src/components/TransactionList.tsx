...
export const TransactionList: React.FC<TransactionListProps> = ({ transactions, accounts }) => {
  ...
  // 🔹 On mount: load stored history
  useEffect(() => {
    setStoredTransactions(loadTransactions());
  }, []);

  // 🔹 Whenever new transactions come from props, merge + save
  useEffect(() => {
    if (transactions.length > 0) {
      const updated = [...storedTransactions, ...transactions];
